...
# Constantes
ANCHO = int(input("Dime el ancho de la cuadrícula"))   # Ancho de la cuadrícula
ALTO = int(input("Dime el alto de la cuadrícula"))  # Alto de la cuadrícula

VIVO = str(input("Dime el caracter para las celulas vivas"))  # Carácter para la celda viva
    ...
    for x in range(ANCHO):
        for y in range(ALTO):
            ...
            if celulas[(x, y)] == VIVO and (numVecinasVivas == 2
                ...
                siguientesCelulas[(x, y)] = VIVO and (numVecinasVivas >= 2)
            else:
